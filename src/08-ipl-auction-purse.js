...
export function iplAuctionSummary(team, players) {
  // Your code here
  if (!team || typeof team !== "object" || Array.isArray(team) || typeof team.purse !== "number" || team.purse <= 0) return null
  ...
  const costliestPlayer = players.reduce((acc, costliest) => costliest.price > acc.price ? costliest : acc)
  const cheapestPlayer = players.reduce((acc, cheapest) => cheapest.price < acc.price ? cheapest : acc)
  ...
  let isOverBudget;
  if (totalSpent > team.purse) isOverBudget = true
  else isOverBudget = false

  return {
    teamName: team.name,
    totalSpent, remaining, playerCount, costliestPlayer, cheapestPlayer, averagePrice, byRole, isOverBudget
  }
}
